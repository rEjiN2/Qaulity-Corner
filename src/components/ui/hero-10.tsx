      ...
      {images.slice(0, 3).map((src, i) => {
        ...
        return (
          <motion.div
            ...
            variants={fanCard}
            className={cn(
              "relative shrink-0 overflow-hidden rounded-[5px] shadow-xl outline outline-black/10 dark:outline-white/10",
              cardAspect,
              slot.width,
              slot.layout,
            )}
...
export function Hero10({
  ...
  const titleElement = title && (
    <h1
      className={cn(
        "text-foreground font-heading font-normal uppercase tracking-tight text-balance",
        vs.title,
      )}
    >
